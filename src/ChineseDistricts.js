/**
 @description: 中国省市县三级数据
 @link: https://github.com/fengyuanchen/distpicker/blob/master/dist/distpicker.data.js
 @date: 2016-8-9
 @author: Rayleigh(Rayleigh@Nero-zou.com)
 */

export function getSub(parentId) {
    let list = []
    for (let id in ChineseDistricts[parentId]) {
        list.push({
            id,
            text: ChineseDistricts[parentId][id]
        })
    }
    return list
}

export function getSubTexts(parentId) {
    let list = []
    for (let id in ChineseDistricts[parentId]) {
        list.push(ChineseDistricts[parentId][id])
    }
    return list
}
...
export function getDetail(id) {
    ...
    let lev1 = ('' + id).substr(0, 2) + '0000'
    let lev2 = ('' + id).substr(0, 4) + '00'
    if (id == lev1) {
        return {
            id,
            text: ChineseDistricts[86][id],
            lev: 1,
            lev1,
            lev2: 0,
            lev3: 0,
 ...
 * @returns {*}
 */
export function getDetailByText(text, parentId) {
    if (parentId) {
        for (let id in ChineseDistricts[parentId]) {
            if (ChineseDistricts[parentId][id] === text) {
                return {
                    id, text
                }
            }
...
/**
 * 直接获取符合Select组件option的数据
 * @returns {*}
 */
export function getTotalArea() {
    let totalArea = []
    for (let provinceId in ChineseDistricts) {
        let tempProvince = {}
        if (provinceId % 10000 === 0) {
            let tempName = getDetailText("86", provinceId).text, totalCity = []
            for (let cityId in ChineseDistricts[provinceId]) {
                let tempCity = {}, tempArea = [], tempName = getDetailText(provinceId, cityId).text
                for (let areaId in ChineseDistricts[cityId]) {
                    let tempName = getDetailText(cityId, areaId).text
                    tempArea.push(tempName)
                }
                tempCity[tempName] = tempArea
                totalCity.push(tempCity)
            }
            tempProvince[tempName] = totalCity
            totalArea.push(tempProvince)
        }
    }
    return totalArea
}

function getDetailText(parentid, selfid) {
    return {
        text: ChineseDistricts[parentid][selfid],
    }
}
...
const ChineseDistricts = {
    ...
    511000: {511002: "市中区", 511011: "东兴区", 511024: "威远县", 511025: "资中县", 511028: "隆昌市",},
    511100: {
        511102: "市中区",
        511111: "沙湾区",
        511112: "五通桥区",
    ...
    520200: {520201: "钟山区", 520203: "六枝特区", 520221: "水城县", 520222: "盘州市",},
    520300: {
        520302: "红花岗区",
        520303: "汇川区",
        520304: "播州区",
